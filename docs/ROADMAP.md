...
| --- | --- | --- | --- |
| v0.1.0 – v0.5.0 | GLFW backend (zlib), hello-world only — replaced by the SDL3 rewrite at v0.6. No per-version breakdown kept. | — superseded; no path forward kept | superseded |
| **v0.6.0** | SDL3 minimal: `Window` create/destroy + `size`/`setSize`/`setPosition`/`setTitle`/`scaleFactor`/`shouldClose`; event pump (`nextEvent`/`pollAllEvents`/`events`); renderer selection (`vulkan`/`opengl`/`none`); per-OS native handle getters + `requiredVulkanInstanceExtensions()`; minimal key action binding. | ☐ **.1** GL context path on an `.opengl` window via `SDL_GL_*` — `glCreateContext`/`glMakeCurrent`/`glSwapWindow`/`glSetSwapInterval`/`glGetProcAddress`/`glDestroyContext` (`13_gl_context_test.zig`). ☐ **.2** implement `capabilities()` (turns the 3 gated `09_capabilities` tests green). **Minor done when** the 6 GL fns + `capabilities()` no longer `@panic` and their suites are green. | **dev** |
| **v0.7.0** | Action-mapped input + window/mouse control. **Shipped:** `bindAction`/`unbindAction`/`actionPressed`/`actionJustPressed`/`actionJustReleased`; runtime window state (fullscreen/resizable/bordered + `is*`, min/max size + getters, minimize/maximize/restore/raise); mouse capture & cursor (relative mode, grab, warp, global show/hide/visible). | ☐ **.1** Input Mapping Contexts — `pushContext`/`popContext`/`replaceTopContext`/`activeContext`/`isContextActive` (un-skips most of `08_context` + some `07_action`). ☐ **.2** axis modifiers behind `actionValue` (deadzone/smooth/scale/invert). ☐ **.3** synthetic injection (`injectAction`). ☐ **.4** TOML-loadable bindings *(decision: small dep vs. hand-rolled parser)*. **Minor done when** the ~26 skipped input tests are green. | **dev** |
| **v0.8.0** | Device & I/O breadth: gamepad device API (Steam Input mapping) + rumble, sensor (gyro/IMU), haptic, clipboard, filesystem paths, power info, IME / text input. | ☐ **.1** filesystem paths `applicationDataDirectory`/`applicationCacheDirectory` (`SDL_GetPrefPath`/`GetBasePath`) + `openWithSystemDefault` (`SDL_OpenURL`) — all 3 `@panic` today. ☐ **.2** clipboard get/set + IME/text-input (`SDL_StartTextInput`/`Stop`). ☐ **.3** gamepad device open/enumerate/close + rumble + connect/disconnect lifecycle. ☐ **.4** sensor (gyro/IMU), haptic, power info (`SDL_GetPowerInfo`). | planned |
| **v0.9.0** | 2D rendering primitives (`SDL_Renderer`) for the `.none` / HUD path. | ☐ **.1** `SDL_Renderer` clear / rect / line / texture behind a clean Zig API. | planned |
| **v0.10.0** | Default audio backend (`SDL_AudioStream`). | ☐ **.1** `SDL_AudioStream`: open device, queue/stream PCM, load WAV *(decision: add mp3 decoder vs. PCM/WAV-only for 1.0)*. | planned |
| **v1.0.0** | Full API surface stable; capability flags complete; cross-platform validated; API frozen. | ☐ **.1** validate Win32 + Android native-handle getters on-device (return `null` on Linux today). ☐ **.2** macOS (contributor-led): `getCocoaHandle` → `CAMetalLayer`, pairs with the vulkan lib's `createMetalSurface`. ☐ **.3** CI matrix: Linux X11 + Wayland (headless compositor), Windows, Android build. ☐ **.4** wire platform-only `nm` gate (zero `vk*`) as a **hard CI gate**; complete capability-flag divergence docs; pin `castholm/SDL` to a released tag; **freeze the API** → tag **v1.0.0**. | planned |
