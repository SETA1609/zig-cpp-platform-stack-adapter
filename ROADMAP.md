...
| --- | --- | --- |
| **v0.6.0** | SDL3 minimal: `Window` create/destroy, event pump (`nextEvent`/`pollAllEvents`), per-OS native handle getters (`getX11Handle`/`getWaylandHandle`/`getWin32Handle`/`getAndroidHandle`), `requiredVulkanInstanceExtensions()`, minimal key action binding (enough for `menu_pause`→ESC) | **Phase 1** — window opens, Vulkan surface creatable, ESC quits |
